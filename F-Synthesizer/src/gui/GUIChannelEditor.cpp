    ...
    const std::function<void(const char* what, const char* impact, const char* caution)>& updateHoverHelp)
{
    ...
    if (ImGui::CollapsingHeader("Source Details", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        else if (auto* sub = std::get_if<SubtractiveConfig>(&chCfg.source))
        {
            sub->unisonVoices = std::clamp(sub->unisonVoices, 1, 8);
            sub->unisonDetuneCents = std::clamp(sub->unisonDetuneCents, 0.0, 120.0);
            sub->unisonSpread = std::clamp(sub->unisonSpread, 0.0, 1.0);
            sub->subOscLevel = std::clamp(sub->subOscLevel, 0.0, 2.0);
            sub->filterCutoffHz = std::clamp(sub->filterCutoffHz, 10.0, 20000.0);
            sub->filterResonance = std::clamp(sub->filterResonance, 0.1, 18.0);
            sub->filterKeytrack = std::clamp(sub->filterKeytrack, 0.0, 1.0);
            sub->modulation.lfo1.rateHz = std::clamp(sub->modulation.lfo1.rateHz, 0.0, 100.0);
            sub->modulation.lfo1.depth = std::clamp(sub->modulation.lfo1.depth, 0.0, 1.0);
            sub->modulation.env2.attackSec = std::clamp(sub->modulation.env2.attackSec, 0.0, 10.0);
            sub->modulation.env2.decaySec = std::clamp(sub->modulation.env2.decaySec, 0.0, 10.0);
            sub->modulation.env2.sustainLevel = std::clamp(sub->modulation.env2.sustainLevel, 0.0, 1.0);
            sub->modulation.env2.releaseSec = std::clamp(sub->modulation.env2.releaseSec, 0.0, 10.0);
            for (auto& route : sub->modulation.matrix.routes)
            {
                route.amount = std::clamp(route.amount, -1.0, 1.0);
            }

            int idx = WaveToIndex(sub->wave);
            const char* waves[] = { "sine", "square", "saw", "triangle" };
            changed |= ImGui::Combo("Wave", &idx, waves, IM_ARRAYSIZE(waves));
            if (updateHoverHelp) updateHoverHelp("Wave を選択します。", "発振波形キャラクターが変わります。", nullptr);
            sub->wave = WaveFromIndex(idx);

            int unisonVoices = sub->unisonVoices;
            ImGui::SetNextItemWidth(220.0f);
            if (ImGui::SliderInt("Unison Voices", &unisonVoices, 1, 8))
            {
                sub->unisonVoices = unisonVoices;
                changed = true;
            }
            if (updateHoverHelp) updateHoverHelp("Unison Voices を調整します。", "重ねる発音数が変わります。", nullptr);
            ImGui::SetNextItemWidth(220.0f);
            changed |= sliderWaveParam("Unison Detune (cent)", sub->unisonDetuneCents, 0.0f, 120.0f, "%.1f");
            ImGui::SetNextItemWidth(220.0f);
            changed |= sliderWaveParam("Unison Spread", sub->unisonSpread, 0.0f, 1.0f, "%.2f");
            ImGui::SetNextItemWidth(220.0f);
            changed |= sliderWaveParam("Sub Osc Level", sub->subOscLevel, 0.0f, 2.0f, "%.2f");

            const char* filterModes[] = { "bypass", "lowpass", "highpass", "bandpass" };
            int filterModeIdx = 0;
            switch (sub->filterMode)
            {
            case FilterMode::Bypass:   filterModeIdx = 0; break;
            case FilterMode::LowPass:  filterModeIdx = 1; break;
            case FilterMode::HighPass: filterModeIdx = 2; break;
            ...
            if (ImGui::Combo("Filter Mode", &filterModeIdx, filterModes, IM_ARRAYSIZE(filterModes)))
            {
                switch (filterModeIdx)
                {
                case 0: sub->filterMode = FilterMode::Bypass;   break;
                case 1: sub->filterMode = FilterMode::LowPass;  break;
                case 2: sub->filterMode = FilterMode::HighPass; break;
                case 3: sub->filterMode = FilterMode::BandPass; break;
                default: sub->filterMode = FilterMode::LowPass; break;
                }
                changed = true;
            }
            if (updateHoverHelp) updateHoverHelp("Filter Mode を選択します。", "フィルタ種別が変わります。", nullptr);
            if (sub->filterMode != FilterMode::Bypass)
            {
                ImGui::SameLine();
                ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.2f, 1.0f), "(active)");
            }
            ImGui::SetNextItemWidth(220.0f);
            changed |= sliderWaveParam("Filter Cutoff (Hz)", sub->filterCutoffHz, 10.0f, 20000.0f, "%.1f");
            if (updateHoverHelp) updateHoverHelp("Filter Cutoff を調整します。", "カットオフ周波数が変わります。", nullptr);
            ImGui::SetNextItemWidth(220.0f);
            changed |= sliderWaveParam("Filter Resonance (Q)", sub->filterResonance, 0.1f, 18.0f, "%.2f");
            if (updateHoverHelp) updateHoverHelp("Filter Resonance を調整します。", "カットオフ付近の強調量が変わります。", nullptr);
            ImGui::SetNextItemWidth(220.0f);
            changed |= sliderWaveParam("Filter Keytrack", sub->filterKeytrack, 0.0f, 1.0f, "%.2f");
            if (updateHoverHelp) updateHoverHelp("Filter Keytrack を調整します。", "ノート音程に連動してカットオフが動く量が変わります。基準は C4(60)。", nullptr);

            // Env2 -> FilterCutoff がフィルタエンベロープを担う。
            changed |= drawModulationEditor("subtractive_modulation", sub->modulation, true, false);
        }
