...
Last Updated: 2026-03-29 (GUI レイアウト修正: Sound 上部を横幅活用レイアウトへ再編)
Branch: `main`
...
- Phase 3-D A/B/F 実装: UI テーマに `CRT Night` を追加し、ヘッダに `Theme` コンボ + `CRT FX`（Scanline/Dot/Vignette）トグルを追加。フレーム毎のテーマ適用と fullscreen オーバーレイ描画で Light CRT（A）/Classic CRT（B）/Theme Stack（F）を統合
