...
MIDIRawOutput ParseSMFFile(const std::filesystem::path& path, int targetChannel)
{
    ...
    MIDIHeader header{};
    if (!ReadHeader(in, header, result.stats)) return result;
    result.ticksPerQuarter = header.division;

    int nextNoteInstanceID = 1;
    for (uint16_t t = 0; t < header.numTracks; t++)
    {
        std::vector<unsigned char> data;
        if (!ReadTrackChunk(in, data)) return result;
        ParseTrack(data, targetChannel, result.rawEvents, result.tempoEvents, result.stats, nextNoteInstanceID);
    }

    result.ok = !result.rawEvents.empty();
    return result;
}
