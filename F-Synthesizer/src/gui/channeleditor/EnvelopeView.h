...
inline bool DrawADSRPreview(const char *id, float attackSec, float decaySec, float sustainLevel, float releaseSec,
                            float curve = 0.0f, float *outAttackSec = nullptr, float *outDecaySec = nullptr,
                            float *outSustainLevel = nullptr, float *outReleaseSec = nullptr)
{
    ...
    const bool editable =
        (outAttackSec != nullptr && outDecaySec != nullptr && outSustainLevel != nullptr && outReleaseSec != nullptr);
    ...
    if (editable && ImGui::IsItemActive())
    {
        ...
        {
            const ImVec2 hp[3] = {toCanvas(x1, 1.0f), toCanvas(x2, sv), toCanvas(x3, sv)};
...
inline void DrawLfo1WavePreview(const char *id, LfoWave wave)
{
    constexpr int kN = 128;
    constexpr int kCycles = 3;

    // S&H 表示用固定ステップパターン（代表的なランダムホールド形状）
    constexpr float kSahSteps[8] = {0.70f, -0.30f, 1.00f, -0.80f, 0.20f, -0.65f, 0.90f, -0.10f};
    ...
    for (int i = 0; i < kN; ++i)
    {
        ...
        case LfoWave::Triangle:
            v = (phase < 0.25f) ? (4.0f * phase) : (phase < 0.75f) ? (2.0f - 4.0f * phase) : (4.0f * phase - 4.0f);
            break;
        case LfoWave::Square:
