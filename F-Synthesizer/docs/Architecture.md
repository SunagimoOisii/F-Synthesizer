# Architecture

最終更新: 2026-02-23

このファイルはアーキテクチャの入口です。  
規模拡大に対応するため、詳細は `docs/architecture/` 配下に分割しました。

## Layer View

```mermaid
flowchart LR
    GUI[gui]
    APP[app]
    CORE[core]
    ENGINE[SynthEngine]
    IO[config / midi / io / synth]

    GUI --> APP --> CORE --> ENGINE
    APP --> IO
```

## Folder Snapshot

```text
docs/
  Architecture.md          <- 入口 (このファイル)
  architecture/
    HANDBOOK.md            <- 全体方針と運用ルール
    README.md              <- 全体像と読み順
    module-map.md          <- 責務と依存方向
    runtime-flow.md        <- 実行時データフロー
    gui.md                 <- GUI分割構成
    config-and-io.md       <- Config/I/O境界
```

## Documents

| Doc | 役割 |
|---|---|
| `docs/architecture/HANDBOOK.md` | 全体方針、原則、更新ルール |
| `docs/architecture/README.md` | 全体像と読み順 |
| `docs/architecture/module-map.md` | モジュール責務と依存方向 |
| `docs/architecture/runtime-flow.md` | 実行時データフロー（CLI/GUI共通） |
| `docs/architecture/gui.md` | GUI構成（main/pianoroll分割後） |
| `docs/architecture/config-and-io.md` | Config読込分割と I/O 方針 |

## Read Flow

```mermaid
flowchart TD
    H[HANDBOOK]
    A[module-map]
    B[runtime-flow]
    C[gui]
    D[config-and-io]

    H --> A --> B --> C --> D
```

## Notes

- 依存方向の原則は `gui -> app -> core`（`core` は `gui` 非依存）
- `ConfigLoad` は `src/config/load/` へ分割済み
- GUI本体は `src/gui/main/`、ピアノロールは `src/gui/pianoroll/` へ分割済み

## Auto-Generated

<!-- AUTO-GENERATED:BEGIN -->
### Auto Snapshot

- Generated by `scripts/check.ps1`
- Generated at: 2026-02-24 16:26:53

### Core Files

- `src/SoundGenerate.cpp` (updated: 2026-02-21 17:21:51)
- `src/app/RunDefaults.cpp` (updated: 2026-02-21 17:55:25)
- `src/app/RunExecution.cpp` (updated: 2026-02-21 19:33:26)
- `src/app/RunSave.cpp` (updated: 2026-02-23 22:11:53)
- `src/app/RunStats.cpp` (updated: 2026-02-21 18:18:17)
- `src/midi/MIDIParser.cpp` (updated: 2026-02-23 22:11:53)
- `src/midi/Sequencer.cpp` (updated: 2026-02-23 22:11:53)
- `src/midi/MidiPipeline.cpp` (updated: 2026-02-22 01:21:05)
- `src/core/RenderGateway.cpp` (updated: 2026-02-21 18:10:08)
- `src/SynthEngine/Engine.cpp` (updated: 2026-02-21 18:44:31)
- `src/SynthEngine/Events.cpp` (updated: 2026-02-21 18:09:42)
- `src/SynthEngine/Renderer.cpp` (updated: 2026-02-23 22:11:53)
- `src/SynthEngine/Voices.cpp` (updated: 2026-02-23 22:11:53)
- `src/io/Writer.cpp` (updated: 2026-02-23 22:11:53)
- `include/SynthEngine/SynthEngine.h` (updated: 2026-02-23 22:11:53)
<!-- AUTO-GENERATED:END -->














